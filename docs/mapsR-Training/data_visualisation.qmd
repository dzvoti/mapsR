...
i)  Label it appropriately assuming that these are data for Maize yield in kilograms per hectare for Chitedze experimental station.
...
i)  another plot that seperates the female and male boxplots. 
i)  label the  boxplots appropriately. 
i)  are there any outliers in your data? 
:::
...
i)  label it and adjust the axis and colour appropriately.
:::
...
i)  Create different types of plot using the days and .
:::
...
i)  `boxplot` of `Petal Length`, 
i)  `qq plot` of `Petal Width` and 
i)  a plot of `Sepal Length` against `Petal Length` on the same plot area with equal dimensions.
:::
